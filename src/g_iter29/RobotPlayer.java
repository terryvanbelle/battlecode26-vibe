...
public class RobotPlayer {
    ...
    static boolean rushSignature = false; // Iteration 202, see runRatKing
    ...
    static void runRatKing(RobotController rc) throws GameActionException {
        ...
        // Iteration 202 (TRAINING_LOG.md): ARM THE RING ON CONTACT, FOR GOOD.
        //
        // The ring is what breaks the peace -- triggerTrap credits the backstab
        // to the trap's OWNER, and 10 of 12 peer King-kill losses follow from
        // it. Iteration 199 suppressed it unless an enemy rat reached our King
        // within 50 rounds, and gained +4 peer games (pure_cooperator
        // 23/54 -> 29/54) at the cost of close-spawn drift (wins 4/42 -> 3/42,
        // wipes 14 -> 16) -- rushes that land after round 50 arrived to no ring.
        // Iteration 201 tried building the ring first and retracting it at r100;
        // that was WORSE than the control on peers (74/108, pure_cooperator
        // 21/54), which proves the peace is already broken well before round
        // 100. A ring that exists at all is a ring that gets triggered.
        //
        // So keep the trigger condition but drop the deadline: the ring arms the
        // first time an enemy rat is seen near our King, whenever that happens,
        // and stays armed. A game where that never occurs is a game where the
        // ring was only ever a liability.
        // Measured, all three variants, peers vs the 76/108 control:
        //   arm within 50 rounds (this)      80/108  pure_cooperator 29/54
        //   build then retract at r100       74/108  pure_cooperator 21/54
        //   arm on contact, no deadline      77/108  pure_cooperator 26/54
        // The deadline is doing real work: a ring that is armed late is a ring
        // the opponent has already had reason to trigger, and retracting one at
        // r100 is far too late -- the peace is long broken by then.
        final int RUSH_WINDOW = 20;
        if (rc.getRoundNum() <= RUSH_WINDOW && !rushSignature) {
            for (RobotInfo info : nearby) {
        ...
        final boolean KING_TRAPS_ENABLED = rushSignature || !rc.isCooperation();
