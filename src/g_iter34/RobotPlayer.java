...
public class RobotPlayer {
    ...
    static void runRatKing(RobotController rc) throws GameActionException {
        ...
        //
        // Iteration 251: the reserve's MAGNITUDE is validated, but its UNIT was
        // never chosen. Instrumenting the King's build gate on `closeup` vs
        // `opportunistic` (a game we lose from both sides, on points at round
        // 2000) showed the King cheese-blocked on **83.5% of its turns**, with
        // the treasury pinned at 987-1044 from round 200 to the end -- an
        // equilibrium sitting exactly on this constant. `NO_SLOT` was 0.6% and
        // the cap bound only before round 200, so nothing else is binding.
        //
        // The reserve exists to buy REPLACEMENT RATS, but it is denominated in
        // cheese while a rat's price moves 8x:
        //
        ...
        //
        // So the fixed reserve is most generous exactly when the army is
        // smallest and rebuilding matters most -- backwards. On that closeup
        // game we fell to 4-6 live rats for 800 rounds while sitting on 20+
        // rats' worth of cheese, and `opportunistic` held 15-22 rats and
        // out-collected us 12429 to 8935. Deaths were not the cause: we lost
        // 113 rats to their 164. We simply stopped rebuilding.
        //
        // Denominating the reserve in RATS makes it self-correcting: it relaxes
        // when the army has collapsed (rats are cheap, rebuilding is urgent)
        // and tightens when the army is large (rats are dear, the buffer is
        // real).
        //
        // Dosed on the 216-game peer set against the g_iter33 control. The
        // curve is concave with an interior optimum at 8, and the per-opponent
        // columns show WHY, which is what makes 8 a choice rather than a fit:
        //
        //   reserve        games  swept-win  swept-loss  imme oppo pure rush
        //   fixed 1000       158      52          2        53   29   27   49
        //   14 * ratCost     159      57          6        53   31   26   49
        //    8 * ratCost     162      60          6        51   32   29   50
        //    4 * ratCost     159      58          7        48   36   26   49
        //
        // Shrinking the reserve monotonically HELPS `opportunistic` (29 -> 31
        // -> 32 -> 36; a long economic game rewards rebuilding) and
        // monotonically HURTS `immediate_defector` (53 -> 53 -> 51 -> 48; it
        // attacks from round 1, and a thin treasury cannot replace the losses).
        // 8 is where those two curves cross. That is the same burst-versus-
        // buffer trade-off that sank the eight MAX_POPULATION raises, priced
        // rather than guessed.
        final int RESERVE_RATS = 8;
        final int REPLACEMENT_RESERVE = RESERVE_RATS * rc.getCurrentRatCost();
