...
public class RobotPlayer {
    ...
    static void runBabyRat(RobotController rc) throws GameActionException {
        ...
        if (throwAllyForward(rc, nearby)) return;   // Iteration 71

        explore(rc);
    }

    /**
     * Iteration 71 (TRAINING_LOG.md): use the grab/throw system to buy tempo.
     *
     * Our usage of `canCarryRat`/`throwRat` has been **zero** for the whole
     * project, while the tournament bots lean on it heavily -- on
     * `hatefullattice` `bench_finalist` records 318 grabs and 88 throws to
     * our none, and on `knifefight` their first throw lands at **round 6** on
     * a map where our King dies at round 77.
     *
     * (Terminology: the replay's `RatNap` action is *kid*nap --
     * `schema/RatNap.java` is documented "Kidnap a rat" -- so those counts
     * are grabs, not rest. I had been misreading them as a rest mechanic.)
     *
     * The trade, read out of `InternalRobot` rather than assumed:
     *   gain  -- `TILES_FLOWN_PER_TURN` 2 x `THROW_DURATION` 4 = up to 8
     *            tiles, against 1 tile/turn on foot.
     *   cost  -- the passenger takes `THROW_DAMAGE` 10 on landing and eats
     *            `HIT_GROUND_COOLDOWN` 10 on movement AND action AND turning
     *            (one frozen turn), and `hitGround` runs
     *            `processTrapsAtLocation`, so it can land on a trap.
     *   cost  -- the thrower pays `THROW_RAT_COOLDOWN` 20, i.e. two turns of
     *            action.
     * Eight turns of walking for 10 HP and one frozen turn is a good rate,
     * but it is not free, so this only fires for a healthy passenger.
     *
     * `throwRat()` takes no direction -- it throws along the thrower's
     * facing. That is why this sits *after* `collectCheese`, at the point
     * where a rat with nothing better to do is about to `explore()` along
     * `preferredExploreDir`: `tryMove` has already been turning us that way,
     * so our facing is the direction we want the passenger to travel.
     *
     * Grabbing also requires `canSenseLocation`, i.e. the ally must be
     * inside our 90-degree vision cone, which is a real constraint (see the
     * vision-cone note in Iteration 66's post-mortem).
     */
    static boolean throwAllyForward(RobotController rc, RobotInfo[] nearby) throws GameActionException {
        // Iteration 72: confine throwing to the OPENING.
        //
        // The unrestricted version (Iteration 71) fired hard -- 58 throws and
        // 239 grabs on `hatefullattice`, against `bench_finalist`'s 88/318 --
        // and cut deaths 67 -> 51. It still lost, because
        // `cheeseTransferred` fell 2595 -> 1225, and the game ended sooner
        // (round 875 vs 1175) rather than later.
        //
        // That is the SAME shape as Iteration 69's trap avoidance (deaths
        // -54%, cheese -54%), and the two together identify the real
        // constraint: **with only 4-8 living rats, every rat-turn spent on
        // anything other than collect-and-deliver is a large fraction of our
        // entire economic output.** `bench_finalist` can afford 88 throws
        // because it fields 56 rats; we cannot afford 58 with six. Mechanisms
        // that cost rat-turns are priced per capita, and we are poor.
        //
        // So bound the spend instead of abandoning the mechanism. The
        // opening is where a throw is worth most and costs least: there is
        // barely any cheese on the ground yet, rats are clustered at spawn
        // (so a grab target is actually adjacent and inside the 90-degree
        // cone), and map presence compounds. `bench_finalist`'s first throw
        // lands at round 6.
        //
        // Unlike Iteration 70's radius, this parameter genuinely changes the
        // evaluated condition -- it gates whether the branch runs at all.
        final int THROW_UNTIL_ROUND = 100;
        if (rc.getRoundNum() > THROW_UNTIL_ROUND) return false;
        if (rc.getCarrying() != null) {
            // Already holding someone -- release them along our facing.
            if (rc.canThrowRat()) {
                rc.throwRat();
                rc.setIndicatorString("threw ally " + rc.getDirection());
                return true;
            }
            return false;
        }
